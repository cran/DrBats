...
Now we fit a [Stan](http://mc-stan.org/) model for the simulated data with a [No-U-Turn Sampler](http://arxiv.org/pdf/1111.4246v1.pdf) using the `modelFit()` function, and the [rstan](https://cran.r-project.org/package=rstan) package.
...
For evaluation, we can convert the object to an `mcmc.list` to apply the diagnostic tests in [the coda package manual](https://cran.r-project.org/web/packages/coda/coda.pdf).  Our package also works on `mcmc.lists` for coherence.

```{r, echo = F}
codafit <- coda.obj(stanfit)
```

If post-processing is needed, our package provides a function to repair reflection problems. Refer to the `modelEval.pdf` vignette for details.
